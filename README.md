...
> 4. 目前本程序在杂交的只会保留视频A的音轨，视频B的音轨将会被舍弃。
...
请先确认输入视频的格式是否被本程序支持，且视频播放不存在问题，硬件加速确保选择正确，如你是AMD显卡就选择AMD AMF。如果选择正确了还是不行就禁用硬件加速。
...
解决方法：~~换设备（bushi），勾选程序中的 “压缩” 选项，使视频的码率降低，减轻设备负担~~ 后续将添加调整码率的选项，敬请期待。
